...
contract LedgerTokenFactoryView is Dispatchable {
    function signatures() external pure override returns (string[] memory _signatures) {
        ...
        _signatures[2] = "predictReceiptToken(string,string,uint8,string)";
    }
    ...
    function selectors() external pure override returns (bytes4[] memory _selectors) {
        ...
        _selectors[n++] = bytes4(keccak256("predictReceiptToken(string,string,uint8,string)"));
    ...
    function predictReceiptToken(string memory name_, string memory symbol_, uint8 decimals_, string memory version_)
        ...
        returns (address)
    {
        return LedgerTokenFactoryLib.predictReceiptToken(_tokenMetadata(name_, symbol_, decimals_, version_));
    }
