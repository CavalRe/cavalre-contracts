...
`LedgerLib.transfer` settles each affected staking program through the Dispatcher-only `settleStakeTransfer` selector before posting, including internal custody paths. Do not duplicate settlement in consuming modules. Same-program transfers settle the sender's exit before the recipient's entry: proportional pending units are redistributed to remaining stake, including the recipient's old balance but excluding incoming principal. Reward-share supply and custody stay unchanged. Full-supply transfers apply the existing final-staker release to the sender. Final release tests remaining stake, not reward-unit ownership. Available rewards stay with their owner. Each debit leaf remains a separate reward position. SR creation rejects nested SR stake/reward assets in either creation order, including registered SR wrapper roots. Ordinary nested Ledger accounts remain supported.
