...
`LedgerTokenFactory` owns program creation; `StakingRewards` owns runtime accounting and views. Install both through Dispatcher. `StakingRewardsToken` presents actual balances beneath a configured staking group; there is no principal receipt ledger. Reward ShareToken supply is the authoritative aggregate outstanding-unit balance, held in one custody leaf until claims.

`LedgerLib` is foundational accounting and must not import SR code, detect staking programs, or invoke reward-settlement hooks. Ordinary Ledger/ERC20 interfaces and token-custody utilities remain valid dependencies. SR settlement belongs above Ledger. WIP: the previous Ledger hook has been removed; replacement settlement is not yet implemented. Do not restore the hook to fix failing SR tests. Existing function selectors and ERC-7201 storage namespaces are preserved during the contract rename. The intended SR behavior remains: Same-program transfers settle the sender's exit before the recipient's entry: proportional pending units are redistributed to remaining stake, including the recipient's old balance but excluding incoming principal. Reward-share supply and custody stay unchanged. Full-supply transfers apply the existing final-staker release to the sender. Final release tests remaining stake, not reward-unit ownership. Available rewards stay with their owner. Each debit leaf remains a separate reward position. SR creation rejects nested SR stake/reward assets in either creation order, including registered SR wrapper roots. Ordinary nested Ledger accounts remain supported.

Storage layouts remain unchanged. The existing checkpoint at the staking-group address records allocation residual units; groups cannot be holders. Final unstake or full-supply transfer credits that residual to the outgoing position. See `modules/staking/README.md` for arithmetic bounds, custody projection, access restrictions and empty-state policies.
