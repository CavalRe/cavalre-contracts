...
`LedgerLib.transfer` settles each affected staking program through the Dispatcher-only `settleStakeTransfer` selector before posting, including internal custody paths. Do not duplicate settlement in consuming modules. Same-program transfers carry proportional pending units; exits allocate forfeited pending units to remaining stake without changing reward-share supply or custody. Final release tests remaining stake, not reward-unit ownership. Available rewards stay with their owner. SR creation rejects nested SR stake/reward assets in either creation order, including registered SR wrapper roots. Ordinary nested Ledger accounts remain supported.
